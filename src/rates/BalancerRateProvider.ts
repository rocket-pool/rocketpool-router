...
class BalancerRateProvider extends RateProvider {
  ...
  private poolPairDataTo: any;
  private poolPairDataFrom: any;
  ...
  async getRethOut(ethIn: BigNumber): Promise<BigNumber> {
    ...
    // Lazy initialise pair data
    if (!this.poolPairDataTo) {
      this.poolPairDataTo = this.msPool.parsePoolPairData(
    ...
    let amountInBN = new OldBigNumber(ethIn.toString());
    let amountInEthBN = amountInBN.div(new OldBigNumber(ONE));
    let rateBN = this.msPool._exactTokenInForTokenOut(this.poolPairDataTo, amountInEthBN);

    let rateEthBN = rateBN.times(new OldBigNumber(ONE));
    return BigNumber.from(rateEthBN.toString());
  }

  async getEthOut(rethIn: BigNumber): Promise<BigNumber> {
    // Lazy initialise the pool instance
    if (!this.msPool) {
      const balancer = new BalancerSDK(this.balanceSdkConfig);
      const pool = await balancer.pools.find(this.poolId);
    ...
    // Lazy initialise pair data
    if (!this.poolPairDataFrom) {
      this.poolPairDataFrom = this.msPool.parsePoolPairData(
        "0xae78736Cd615f374D3085123A210448E74Fc6393",
        "0xc02aaa39b223fe8d0a0e5c4f27ead9083c756cc2"
      );
    }
    ...
    let amountInBN = new OldBigNumber(rethIn.toString());
    let amountInEthBN = amountInBN.div(new OldBigNumber(ONE));
    let rateBN = this.msPool._exactTokenInForTokenOut(this.poolPairDataFrom, amountInEthBN);
